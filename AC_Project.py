...
blue_range = np.array([[88, 78, 26], [128, 255, 255]])
yellow_range = np.array([[21, 76, 80], [61, 255, 255]])
red_range = np.array([[158, 85, 72], [180, 255, 255]])
b_cen, y_pos, r_cen = [240, 326], [240, 320], [240, 320]
...
def distance(cl, c2):
    distance = pow(pow(c1[0]-c2[0], 2) + pow(cl[1]-c2[1], 2), 0.5)
    return distance
...
def performAction(yp, rc, bc, action, drag, perform):
    if perform:
        ...
        while (1):
            ...
            changeStatus(k)
            _, frameinv = cap.read()
            # flip para el modo espejo en la camara
            frame = cv2.flip(frameinv, 1)
            hsv = cw2.cvtColor(frame, cv2.COLOR_BGR2HSV)
